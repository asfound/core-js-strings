...
function sumOfCodes(str) {
  if (!str) return '';
  const arr = str.split('');
  return arr.reduce((sum, current) => sum + current.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  return arr.reduce((acc, current) => (v.includes(current) ? acc + 1 : acc), 0);
}
...
function findLongestWord(sentence) {
  ...
  return arr.toSorted((a, b) => b.length - a.length)[0];
}
...
function encodeToRot13(str) {
  const encoded = str.split('').map((char) => char.charCodeAt());
  const decoded = encoded.map((code) => {
...
function getCardId(value) {
  ...
  if (value.includes('♥')) i = 2;
  if (value.includes('♠')) i = 3;
